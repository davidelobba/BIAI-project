...
def cppn_fitness(cppn, target_network, width=56, height=56):
    # Generate image using CPPN
    image = np.zeros((3, width, height))
    for x in range(width):
        for y in range(height):
            coord = torch.tensor([x / width, y / height]).float()
            image[:, x, y] = cppn(coord).detach().numpy()
    
    # Upsample the image to match the network's expected input size
    image = upsample_numpy_image(image)

    # Evaluate using the target neural network
    label, confidence = get_classification_and_confidence(image, target_network)

    # Here, the fitness could be the misclassification confidence
    return confidence if label != true_label else 0
